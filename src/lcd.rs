...
pub async fn manage_display(mut display: GraphicsMode<I2CInterface<I2c<'static, Async, Master>>>) {
    ...
    loop {
        ...
        match button {
            b @ Button1 => {
                info!("LCD task received button press: {:?}", b);
                Text::with_baseline("Button 1 pressed", Point::zero(), text_style, Baseline::Top)
                    .draw(&mut display)
                    .unwrap();
            ...
            b @ Button2 => {
                info!("LCD task received button press: {:?}", b);
                Text::with_baseline("Button 2 pressed", Point::zero(), text_style, Baseline::Top)
                    .draw(&mut display)
                    .unwrap();
                display.flush().unwrap();
            }
            b @ Button3 => {
                info!("LCD task received button press: {:?}", b);
                Text::with_baseline("Button 3 pressed", Point::zero(), text_style, Baseline::Top)
                    .draw(&mut display)
                    .unwrap();
                display.flush().unwrap();
            }
